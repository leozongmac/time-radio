...
首次运行前，在终端进入项目目录并赋予两个脚本执行权限：

```bash
chmod +x 安装时光收音机依赖.command
chmod +x 启动时光收音机.command
```

先双击 `安装时光收音机依赖.command` 安装依赖，再双击 `启动时光收音机.command` 启动应用；也可以在终端执行：
...
依赖安装脚本只负责创建 `.venv-web` 并安装 `pyproject.toml` 中的运行依赖，不会启动后端；它会优先使用 `uv`，没有 `uv` 时寻找 Python 3.11 或 3.12。启动脚本会复用同一个环境，默认地址为 `http://127.0.0.1:8766`。
